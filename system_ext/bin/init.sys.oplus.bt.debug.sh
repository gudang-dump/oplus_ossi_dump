...
#ifdef OPLUS_FEATURE_BT_SWITCH_LOG
#Liusuqi@CONNECTIVITY.BT.Basic.Log.3610662, 2022/06/18, add for rm sysTrace when err donot happened
function removeSysTrace() {
    sysTracePath="/data/local/traces"
    needCatchTrace=`getprop persist.sys.oplus.bt.switch_log.catch_trace`
    if [ "x${needCatchTrace}" == "xtrue" ];then
        traceFilename=`ls -t ${sysTracePath}|head -n1|awk '{print $0}'`
        # if bt switch err not occured, rm the trace file.
        rm ${sysTracePath}/${traceFilename}
        setprop sys.oplus.bt.switch.err_check 0
    fi
}
...
function packBtSwitchLog() {
    ...
    if [ "x${needCatchTrace}" == "xtrue" ];then
        traceFilename=`ls -t ${sysTracePath}|head -n1|awk '{print $0}'`
        # if bt swithch err occured, move the trace file to btSwitchLogPath.
        mv ${sysTracePath}/${traceFilename} ${btSwitchLogPath}
    fi
