...
function collectwifidmesg(){
    ...
    platform=`getprop ro.board.platform`
    needKernelLog=`getprop ro.oplus.minidump.kernel.log.support`
    country=`getprop persist.sys.oplus.region`
    echo "needKernelLog : ${needKernelLog}"
    ...
    if [ $dewifidumpcount -lt 10 ] && [ $enwifidumpcount -lt 10 ] && [ $needUpload -eq 1 ];then
        if [ "x${country}" == "xOC" ] || [ "x${country}" == "xCN" ] || [ "x${country}" == "xIN" ] || [ "x${country}" == "xSG" ];then
            if [ "x${needKernelLog}" == "xtrue" ];then
                echo "kernel log added"
